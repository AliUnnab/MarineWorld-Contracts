...
const okf_converter_1 = require("./okf-converter");
...
const getDriveService = () => _driveService || (_driveService = new drive_service_1.DriveService());
let _parserService = null;
const getParserService = () => _parserService || (_parserService = new parser_service_1.ParserService());
let _okfConverter = null;
const getOkfConverter = () => _okfConverter || (_okfConverter = new okf_converter_1.OKFConverter());
let _githubService = null;
const getGithubService = () => _githubService || (_githubService = new github_service_1.GitHubService());
let _vectorService = null;
const getVectorService = () => _vectorService || (_vectorService = new vector_service_1.VectorService());
...
router.get("/auth/google/callback", async (req, res) => {
    ...
    try {
        ...
        const userDriveService = new drive_service_1.DriveService(tokenToUse);
...
router.post("/drive-webhook", async (req, res) => {
    ...
    const token = req.headers["x-goog-channel-token"]; // Contains metadata e.g. companyId=XYZ
...
router.post("/agent/query", async (req, res) => {
    ...
    try {
        const results = await getAgentService().queryByAgentRole(role, companyId, query, limit ? parseInt(limit.toString()) : 5);
        return res.json({
            success: true,
            role,
            companyId,
 ...
 */
async function runProcessingPipeline(companyId, folderId, specificFileId) {
    const db = (0, firestore_1.getFirestore)("ai-studio-6dbfd403-b57c-4e02-8999-633ee65aff51");
    const companyDoc = await db.collection("companies").doc(companyId).get();
    ...
    if (companyDoc.exists) {
        ...
        if (refreshToken) {
            companyDriveService = new drive_service_1.DriveService(refreshToken);
        }
    }
    const drive = companyDriveService.drive;
    if (!drive) {
    ...
    // 2. Download File
    const fileExtension = path_1.default.extname(targetFileName) || getExtensionFromMime(targetMimeType);
    const localFileName = `${targetFileId}${fileExtension}`;
    const localFilePath = path_1.default.join(TMP_DIR, localFileName);
    try {
        ...
        const gitResult = await getGithubService().commitOKFDocument(companyId, targetFileName, okfDoc.rawContent, `[OKF Sync] Auto-update corporate file: ${targetFileName} (Type: ${okfDoc.metadata.type})`);
        // 6. Vector Index the markdown content
        console.log("🗂️ Ingesting document chunks into Firestore Vector Index...");
        const chunkCount = await getVectorService().indexDocument(companyId, targetFileName, okfDoc.metadata.type, okfDoc.markdownBody, okfDoc.metadata.tags, okfDoc.metadata.last_updated);
        return {
            status: "completed",
