...
class BackupSyncService {
     ...
     */
    async getCompanyFolderId(companyId, driveService) {
        try {
     ...
     */
    async getCachedPdfBuffer(contractId) {
        try {
            ...
            if (cachedData.isChunked) {
                ...
                const chunksSnap = await this.db.collection("contract_pdfs")
                    ...
                    .orderBy("index")
                    .get();
                const base64Parts = chunksSnap.docs.map((d) => d.data().chunkData);
     ...
     */
    generateContractPDF(contractId, data) {
        ...
        const drawText = (text, fontSize, isBold, color = [15, 23, 42], align = 'left', customLineHeight) => {
        ...
        if (data.additionalParties && data.additionalParties.length > 0) {
            ...
            y += 2;
            data.additionalParties.forEach((p, idx) => {
