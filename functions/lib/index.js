...
const pipeline_router_1 = __importDefault(require("./okf-pipeline/pipeline-router"));
admin.initializeApp();
const dbAdmin = (0, firestore_1.getFirestore)(admin.app(), "ai-studio-6dbfd403-b57c-4e02-8999-633ee65aff51");
...
function generateInvoicePDF(invoiceNumber, plan, amount, date, customerEmail, customerName, workspaceId, paymentMethod, transactionId) {
    const doc = new jspdf_1.jsPDF({ format: 'a5', orientation: 'portrait' });
...
app.get("/api/stripe/customer/:customerId", async (req, res) => {
    try {
        ...
        const customer = await stripeInstance.customers.retrieve(req.params.customerId);
        res.json({ customer });
    }
    catch (error) {
...
app.get("/api/stripe/subscriptions/:customerId", async (req, res) => {
    try {
        const stripeInstance = getStripe();
        const subscriptions = await stripeInstance.subscriptions.list({ customer: req.params.customerId });
...
app.get("/api/stripe/invoices/:customerId", async (req, res) => {
    try {
        const stripeInstance = getStripe();
        const invoices = await stripeInstance.invoices.list({ customer: req.params.customerId, limit: 10 });
...
app.get("/api/stripe/payment-methods/:customerId", async (req, res) => {
    try {
        const stripeInstance = getStripe();
        const paymentMethods = await stripeInstance.paymentMethods.list({ customer: req.params.customerId });
        ...
        const session = await stripeInstance.checkout.sessions.create({
            ...
            line_items: [{
                    price_data: {
                        ...
                        unit_amount: Math.round(parseFloat(priceAmount.toString().replace('$', '')) * 100),
        ...
        if (session.payment_status === 'paid') {
            ...
            if (emailTo) {
                ...
                const transactionId = session.payment_intent
                    ? `TX-${session.payment_intent.replace('pi_', '').substring(0, 8).toUpperCase()}`
                    : 'TX-EEBZ5C9M';
                try {
                    const pdfBuffer = generateInvoicePDF(invoiceNumber, planName, amountStr, now.toISOString(), emailTo, customerName, workspaceId, paymentMethod, transactionId);
                    const mailOptions = {
...
// ------------------------------------------
// DOCK VERIFICATION & RESTORATION MAILS
// ------------------------------------------
...
app.post("/api/auth/reset-password", async (req, res) => {
    try {
        ...
        await (0, auth_1.getAuth)().updateUser(userRecord.uid, {
            password: password
        });
        // Mark token as used
...
app.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        if (!key || !webhookSecret) {
            throw new Error('Stripe configuration is missing in environment.');
        }
        const stripeInstance = new stripe_1.default(key, {
    ...
    switch (event.type) {
        case 'checkout.session.completed':
            ...
            try {
                await dbAdmin.collection('pending_stripe_checkout_sessions').doc(session.id || 'unknown_session').set({
                    status: 'completed',
