...
const newProcessLegalText = `export const processLegalText = (text: string, startNumber: number = 1): string => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (numberMatch) {
      ...
      // Determine heading level based on number of parts
      if (thirdNum !== undefined) {
         // e.g. 1.1.1. -> Level 3 clause -> use h5
         processedLines.push(\`\\n##### \${newNumStr} \${formattedContent}\\n\`);
      } else if (secondNum !== undefined) {
         // e.g. 1.1. -> Level 2 clause -> use h4
         processedLines.push(\`\\n#### \${newNumStr} \${formattedContent}\\n\`);
      } else {
         // e.g. 1. -> Level 1 clause -> use h3
         processedLines.push(\`\\n### \${newNumStr} \${formattedContent}\\n\`);
      }
      continue;
    }
